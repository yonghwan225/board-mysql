...
@DataJpaTest  // 레포지터리와 테스트를 하겠다 명시
...
//        Comment fir = new Comment(7L, article, "Rora", "청소잘함");
//        Comment sec = new Comment(8L, article, "Carten", "야근잘함");
//        Comment thr = new Comment(9L, article, "Aiden", "롤 잘함");
...
//    }
    ...
    @Test
    @DisplayName("1번게시글_조회하여_테스트") //테스트 결과 이름 출력
    void findByArticleId() {
        //6번 게시글 조회하는 댓글 조회
        Long articleId = 1L;
        //1. expected - 예상 데이터 작성
    ...
    void findByNickname() {
        ...
        Comment fir = new Comment(2L,
                new Article(4L, "나의 최애는?", "ㄱㄱㄱ"),
                nickname, "심형래");
        Comment sec = new Comment(4L,
                new Article(5L, "나의 인생 언어는?", "댓 ㄱㄱ"),
                nickname, "아프리카어");
        Comment thr = new Comment(9L,
                new Article(6L, "나의 장점은?", "대댓 ㄱㄱ"),
                nickname, "롤 잘함");
        List<Comment> expected = Arrays.asList(fir, sec, thr);
